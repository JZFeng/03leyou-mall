...
public class GoodsService {
    ...
    public void save(SpuBo spu) {
        ...
        //发送消息到消息队列
        asyncSendMessage(spu.getId(), "insert");
    }
    ...
    public void updateGoods(SpuBo spu) {
        ...
        //发送消息到消息队列
        this.asyncSendMessage(spu.getId(), "update");
    ...
    private void sendMessage(Long spuId, String msgType) {
        try {
            this.amqpTemplate.convertAndSend("item." + msgType, spuId);
        } catch (Exception e) {
            logger.error("{}商品消息发送异常，商品Id:{}", msgType, spuId, e );
        }
    }

    public void asyncSendMessage(Long spuId, String msgType) {
        ThreadUtils.execute(() ->{
            System.out.println("正在发送消息， spuId: " + spuId + ", Msg Type:" + msgType );
